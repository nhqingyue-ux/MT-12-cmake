...
extern volatile unsigned short thermal_PosAndNeg[3][4];  // Fu 2018/12/10 : �O�����t�ūת��A
	...
	switch(SetNumber)
	{
		case 0x01:
			...
			for(i=0; i<12; i++)
			{
				...
				*(tempData[i].HeatWattUnit) = 0;	// Fu 108/12/30 : �[�J�ƥ��C�q�ūת��[���˯S��
				if(i < 7)
				{
					*tempData[i].HeatTm = 0;
...
void TpTypeMdSet(void)
{
	...
	*(tempData[0].TempLinearErr) = OFF;
	...
	for(i=0; i<12; i++)
	{
		for(j=4; j<(ch1_sec_num[i]-1); j++)			// �ҩl�q = 0 ��
		{
			if(ch1[i][j]  == 0xffff)
			{
...
void CheckEEPROM(unsigned short CheckNumber)
{
	...
	switch(CheckNumber)
	{
		case 1:
			...
			//
			for(CheckEEPROM_i = 0; CheckEEPROM_i < 499; CheckEEPROM_i++) // �ˬdEEPROM�O�_����0
			{
				if(*GetPtrCDM2(20001+CheckEEPROM_i) != 0)
				{
					*GetPtrCDM2(20416) = 0xAAAA; // EEPROM����0, �h���AAAA
					break;
				}
			}
			//
