...
void TempDisplay(void)
{
    ...
    //
    *(tempData[0].HiAlmSts) = HiAlmSts;  // é«˜æº«åå·®ç‹€æ…‹BIT -
    *(tempData[0].LowAlmSts) = LowAlmSts; // ä½Žæº«åå·®ç‹€æ…‹BIT
    //
    sts = 0;
    Heat_Central = 5050;
    if((LowAlmSts & 0x07FF7FF) && ((*(tempData[0].HeatCtrlMd) & 0x0007)==1))
        sts |= 0x01;                        // (ä¸åŠ ç†±)ä¸‹SETæ–™æº«éŽlowç‹€æ…‹
    
	if(HiAlmSts & 0x07FF7FF)
        sts |= 0x02;                        // (ä¸åŠ ç†±)ä¸‹SETæ–™æº«éŽhighç‹€æ…‹ sts = state
    //
    if((*(tempData[0].HeatCtrlMd) & 0x000f)==0)				// 6547é›»ç†±æŽ§åˆ¶(æŽ§åˆ¶é›»ç£æŽ¥è§¸å™¨)(Heater-Control)
    {
        TpClLpFg4 = 0;
        InitTpHt = 0;
        TwoTpFg = 0;
        TwoTpHeatFg = 0;
				sts = sts & 0x02;                   // ä¸åŠ ç†±ä¸‹æ¸…é™¤æ–™æº«éŽä½Žç‹€æ…‹
	}
    //
    ...
    //
    if(TpOkFg == 0)
        sts = sts & 0x02;				// æœªåˆ°é”æ–™æº«æ™‚æ¸…é™¤æ–™æº«éŽä½Žç‹€æ…‹
    //
    ...
    if(PID_CAN_CTRL_FG>100)
    {
        ...
        else
        {
            if(heatmode==1)
            {
                if(!(BK_sts))
                {
                    TpOkFg = ON;                        // æ–™æº«æ­£å¸¸(Temperature-Normal)
                }
            }
            TempHiFg = 0;
            TempLowFg = 0;
