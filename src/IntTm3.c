	...
	if((SendRs485Fg == 0) && (dev_addr == id))
	{
		switch(func_code)
		{
			case 0x33:  // 3 of function code represents query user-modified data(system parameters) // RD
				...
				length = DataLenCnt;
				crc_temp = CRC16((unsigned char *)rx_buffer, 6);	// 包含FUN & ID & 長度
				...
				if(crc_temp == crc_temp_Bk)
				{
					...
					tx_buffer[2] = (unsigned char)length;  // Byte of length	 不包含FUN & ID & 長度
					DataLenCnt = length;  // Byte of length	 不包含FUN & ID & 長度(address + data = 1 word)
					//
					...
					if(AddressSetBk <= AddressBase0)
					{
						...
						crc_temp = CRC16((unsigned char *)tx_buffer, (DataLenCnt*2)+3); 	// 資料 + ID + FUN + 長度
					...
					else
					{
						...
						crc_temp = CRC16((unsigned char *)tx_buffer, DataLenCnt+3); 	// 資料 + ID + FUN + 長度
				...
				if(crc_temp == crc_temp_Bk)
				{
					...
					if(AddressSetBk <= AddressBase0)
					{
						...
						*GetPtrCDM2(AddressSetBk) = DataSetBk;
						...
						//
						crc_temp = CRC16((unsigned char *)tx_buffer, 6); 	// 資料 + ID + FUN + 長度
...
//				SendRs485Fg = 1;  // 2018/02/26 by kf
			...
			case 0x13:  // 多筆資料寫入// WR
				...
				DataLenCnt =  (rx_buffer[4]<<8) | rx_buffer[5];
				length = DataLenCnt;
				crc_temp = CRC16((unsigned char *)rx_buffer, 7+rx_buffer[6]);	// 包含FUN & ID & 長度
				...
				if(crc_temp == crc_temp_Bk)
				{
					...
					crc_temp = CRC16((unsigned char *)tx_buffer, 6); 	// 資料 + ID + FUN + Address + 長度
					tx_buffer[6] = crc_temp & 0x00ff;
					tx_buffer[7] = (crc_temp & 0xff00)>>8;
					tx_buffer[8] = 0;
				}
...
//				{  // 2012/09/29
					...
					length = DataLenCnt;
					crc_temp = CRC16((unsigned char *)rx_buffer, length+4);	// 包含FUN & ID & 長度
...
//				SendRs485Fg = 1;  // 2018/02/26 by kf
			...
			case 0xEFu :
				...
				DataLenCnt =  (rx_buffer[2]<<8) | rx_buffer[3];
				length = DataLenCnt;
				crc_temp = CRC16((unsigned char *)rx_buffer, 4);	// 包含FUN & ID & 長度
				...
				if(crc_temp == crc_temp_Bk)
				{
					...
					tx_buffer[2u] = 0x10u;  // Byte of length	 不包含FUN & ID & 長度
					...
					tx_buffer[18u] = '0';
					//
					crc_temp = CRC16((unsigned char *)tx_buffer, (16u+3u)); 	// 資料 + ID + FUN + 長度
