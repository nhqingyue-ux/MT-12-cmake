...
#define REG_RES_SIZE	((0x3000/2)-(T_MAX*3+C_MAX*4)-3)	//³ÑŸl(«O¯d)
//
#define Y_offset	0
...
//
#define MCSJ_MAX		1024			// WMLAD¥iœs¿èšÏ¥ÎMCSJ€§³Ì€jŒÆ
#define OSH_TMP_SIZE		(Y_MAX+M_MAX)	// >(Y+M)
#define STATE_TMP_SIZE		(M_MAX)			// >(M)
#define MCSJ_TMP_SIZE		(MCSJ_MAX)		// > MCSJ (šÌWMLAD­­šî³Ì€jšÏ¥ÎŒÆ)
#define LAD_STACK_SIZE		256				// unsigned short,€@¯ë°ïÅ|°Ï
#define MCS_STACK_SIZE		256				// unsigned short,MCSJ°ïÅ|°Ï
...
{										
	...
	unsigned short StsRes[STS_RES_SIZE/16];			// ª¬ºABIT«O¯d°Ï, 1100~2047 unsigned shorts (0x044C ~ 0x07FF)

	// *** Ladder°ÑŒÆRegister°Ï 0x1000~0x3fff
	//0x1000 bytes
	...
	unsigned short RegRes[REG_RES_SIZE-1];			// žê®ÆREG«O¯d°Ï
	...
	//0x3FFF ----End
	//
	// *** Ladder OneShot&°ïÅ|°Ï 0x4000~, total
	//0x4000
	unsigned short OshTmp[OSH_TMP_SIZE/16];			// ©ñžmCoil(Y,M)ª¬ºAÅÜ€Æ
	unsigned short StateTmp[STATE_TMP_SIZE/16];		// ©ñžmStateª¬ºAÅÜ€Æ
	unsigned short McsjTmp[MCSJ_TMP_SIZE/16];		// ©ñžmMCSJª¬ºAÅÜ€Æ
	unsigned short LadStack[LAD_STACK_SIZE];		// Ladder ª¬ºA°ïÅ|°Ï
	unsigned short McsStack[MCS_STACK_SIZE];		// MCSJ»PSTATEªºD1°ïÅ|°Ï
	// ·sŒW¥[ªº¥ÑŠ¹«Ø¥ß
};*/
//
//
...
struct TpPID_DATA
{
  ...
  unsigned short  *PwmOutput;					// 52
	...
	unsigned short	*HeatWattUnit;	// Fu 108/12/30 : ¥[¤J³Æ¥÷¨C¬q·Å«×ªº¥[¼ö¥Ë¯S¼Æ
	//
