...
extern float OutputPwmMaxUnit[12];	// Fu 108/01/03 : •—≥Ê¨q≈‹ßÛ¨∞¶h¨q
...
void TempSubScan(void)
{
    ...
    for(i=0; i<TpMaxLp; i++)
    {
		...
		*(tempData[i].TpConAddHi) = *(tempData[i].TpControl) +  *(tempData[i].HiAlarm);
		...
		//
		if(!(TwoHeatMdFinishFlag & (0x01<<i)) && (*(tempData[0].TwoHeatMd) & (0x01<<i)) && (TwoHeatSetTm[i] == (Tm1Sec1_TRIG_MS * *(tempData[i].TwoHeatTm))) && (*(tempData[i].TpControl) == OldTpControl[i])) // 2016/02/04 §@´ˆ•[ºˆ, ≠Y∑≈´◊¶b§Ò®“±a§∫, ßY§£≤M∞£®‚¨q§…∑≈§Œ¶P®B§…∑≈•\Ø‡™∫∞—º∆.
		{
		...
		{
			///// 2014.02.07 •[§J∑Ì∑≈´◊≥]©w≠»§WΩ’´·, P≠»¿Hµ€ª~ÆtßÔ≈‹.
			...
			/////
			if((*(tempData[i].TpControl)-OldTpControl[i]) > BkPidPUnit[i]) // 2015/11/26 ≈‹ßÛ•[ºˆ≠»Ωd≥Ú§j©Û§Ò®“±a, §~∞ı¶Ê®‚¨q∂W∑≈•\Ø‡.
			{
				TwoTpHeatFg &= ~(0x01<<i);		// ≠´∑s≠p∫‚§G¨q§…∑≈•\Ø‡
				TwoTpFg &= ~(0x01<<i);				// ≠´∑s≠p∫‚§G¨q§…∑≈•\Ø‡
		...
		if((Temp) && (*(tempData[i].TpControl) != 0) && (*(tempData[i].TpControl) != 0xffff) && (BkPidPUnit[i] != 0xffff) && (BkPidPUnit[i]) && (TpPIDdata[i].Curr <= 5800) && (SecTempSw & (0x01<<i)))
		{
				...
				if((!(TpClLpFg2 & (0x01<<i))) && (*(tempData[0].AuTnFun) == 0) && !(ThermostatFun & (0x01<<i))) // 2016/01/29 •[§JThermostatFun
				{
						...
						*tempData[i].TwoUpOffset = TpPIDdata[i].P;
						//
						// Fu 108/01/03 : •i•H®C¨qøW•ﬂøÔæ‹MT12º“¶°©Œ¨OMJ86º“¶°
						if((*(tempData[0].ThermostatFun) & 0x8000) && ((*tempData[0].TwoUpFun & (0x01<<i)) == 0))	// 15 Bit : MJ86 - 12¨ÌPIDº“¶°
						{
						...
						else
						{
							if((TwoUpFun & (0x01<<i)) && (((*(tempData[0].TwoHeatMd) & (0x01<<i)) && (TwoHeatSetTm[i] < (Tm1Sec1_TRIG_MS * *(tempData[i].TwoHeatTm))) && (*(tempData[i].TpControl) < 1500)) || !(*(tempData[0].SynchronFun) & (0x01<<i)) || (TpRangCnt >= 48))) // 2016/01/08
							{
									...
									//
									if(TwoUpTempMd(i, TpPIDdata[i].Set, thermal_couple[i], (*tempData[i].TwoUpOffset)))	// §G¨q∂W∑≈•\Ø‡
									{
											TpPIDdata[i].Set = thermal_couple[i]+3;  // ≈‹ßÛ≥]©w¨∞•ÿ´e≠» , •u≥—§UPD≠»
											TpPIDdata[i].Isum1 = TpPIDdata[i].Isum1/10;  // ¥ÓßC∑≈´◊πL∑≈
									}
							}
							else
							{
						...
						{
							//	Fu 104/08/13
							if((*(tempData[0].SynchronFun) & (0x01<<i)) && (Old20515 <= 48) && (TpPIDdata[i].P) && (*(tempData[i].TpControl) >= 1200))		 	// ¶P®B§…∑≈•\Ø‡
							{
						...
						{
							//	Fu 104/08/13
							if((*(tempData[0].SynchronFun) & (0x01<<i)) && (TpRangCnt <= 48) && (TpPIDdata[i].P) && (*(tempData[i].TpControl) >= 1200))		 	// ¶P®B§…∑≈•\Ø‡
							{
								TpRangUsedFg |= (0x01<<i);
								//
						...
						if((*(tempData[0].ThermostatFun) & 0x8000) && ((*tempData[0].TwoUpFun & (0x01<<i)) == 0))	// 15 Bit : MJ86 - 12¨ÌPIDº“¶°
						{
							...
							if(PidMathTime[i] >= 46)	// 11500ms
							{
								...
								TpPIDdata[i].OldCurr = thermal_couple[i];	// πB∫‚§@¶∏PID≠»∞µ§@¶∏∞Oø˝§ß´e™∫∑≈´◊
								PidMathTime[i] = 0;
							}
						...
						//
						//if((*(tempData[0].SynchronFun) & (0x01<<i)) && (i==SynFg) && (SynFg <= 11) && !(TpClLpFg4 & (0x01<<i)))		 	// ¶P®B§…∑≈•\Ø‡
							//TpPIDdata[i].Isum1 = TpPIDdata[i].Isum1/(TpPIDdata[i].P/10.);	// ¥ÓßC∑≈´◊πL∑≈
				}
				//
				if(!(*(tempData[0].AuTnFun) & (0x01<<i)))
				{
					if(AlternTm[i] == 0)
					{
						...
						{	    
							if(*(tempData[0].ContrlFun) & (0x01<<i))    // Relay
							{
								...
								{
									RelayTmMd[i]= 0;
									LastOutUnit1[i] = (OrgTpOutPwmBk[i] * ((*(tempData[i].CycleTm)/10))) / 32767;	   	// •ÿ´e-PID Unit
									LastOutUnit2[i] = (OrgTpOutPwmBk2[i] * ((*(tempData[i].CycleTm)/10))) / 32767;	   	// §ß´e-PID Unit
									if(LastOutUnit1[i] == 0)
					...
					//else    // SSR : 500ms
					//{
						//LastOutUnit1[i] = (OrgTpOutPwmBk[i] * Sec025_TRIG_MS) / 32767;	   	// •ÿ´e
						//LastOutUnit2[i] = (OrgTpOutPwmBk2[i] * Sec025_TRIG_MS) / 32767;	   	// §ß´e
						//if(LastOutUnit1[i] == 0)
							// *GetPtrCDM2(20616+i) = 0;
						//else if(LastOutUnit1[i] > LastOutUnit2[i])
						//{
				...
				//if((TpClLpFg3 & (0x01<<i)) && (TwoUpTm[i] == 20))	// Fu 102/08/02
				{
					...
					{
						TpPIDdata[i].Err = (float)(*(tempData[i].TpControl)) - (float)(Bk_thermal_couple[i]);   // Err = Tset - Tcurr  // Áï∂Ê¨°Ë™§Â∑Æ = Ë®≠ÂÆöÂ£ìÂäõ - ÁõÆÂâçÂ£ìÂäõ
						if(TpPIDdata[i].Err < 0)
							ErrArea = 20;
						else
							ErrArea = 50;
						if(((unsigned short)abs(TpPIDdata[i].Err) > 10) && ((unsigned short)abs(TpPIDdata[i].Err) <= ErrArea))// ∏®©Û2.0´◊§ß∂°§~∞µπB∫‚
						{
							PID2PTmP[i]++;
							if(PID2PTmP[i] >= 120) // 250*120 = 60 sec
							{
								PID2PTmP[i] = 0; 
								if(Bk_thermal_couple[i] > *(tempData[i].TpControl))	// •ÿ´e∑≈´◊∞æ∞™
								{
						...
						//
						TpPIDdata[i].Err = (float)(*(tempData[i].TpControl)) - (float)(Bk_thermal_couple[i]);   // Err = Tset - Tcurr  // Áï∂Ê¨°Ë™§Â∑Æ = Ë®≠ÂÆöÂ£ìÂäõ - ÁõÆÂâçÂ£ìÂäõ
						if(((((unsigned short)abs(TpPIDdata[i].Err) <= 10)) && ((unsigned short)abs(TpPIDdata[i].Err) >= 5)) && (PIDTpCurrBkP[i] == 0))// ∏®©Û0.5 ~ 2.5´◊§ß∂°§~∞µπB∫‚
						{
							PID2PTmI[i]++;
						...
						//	Fu 101/10/19
						//
						if(((unsigned short)abs(TpPIDdata[i].Err) >= 15) && ((unsigned short)abs(TpPIDdata[i].Err) <= BkPidPUnit[i]))// ∏®©Û1.5 ~ 20.0´◊§ß∂°§~∞µπB∫‚
						{
							PID2PTmD[i]++;
			...
			else
			{
					...
					if((*(tempData[0].ThermostatFun) & 0x8000) && ((*tempData[0].TwoUpFun & (0x01<<i)) == 0))	// 15 Bit : MJ86 - 12¨ÌPIDº“¶°
					{
						PidMathTime[i] += 1;
						//if(PidMathTime[i] >= 46)	// 11500ms
						if(PidMathTime[i] >= 8)	// 250ms * 4 = 2 Sec : Fu 108/01/03 : ßP¬_∑≈´◊≈‹ßÛ¨∞2¨Ì∞µ1¶∏
						{
							PidMathTime[i] = 0;
			...
			*GetPtrCDM2(20561) = *tempData[0].AuTnFun;
			...
			if(*tempData[0].AuTnFun)
				...
				*tempData[0].AutoTingSts = 1;	// ∑≈´◊¶€∞ Ω’ø”ßπ¶®
     	}
			else
			{
				...
				//
				TpAuStopFg &= ~(0x01<<i);
				// Fu 109/07/28 : •[§J¶ §¿§ÒøÈ•Xº“¶°§]≠nßP¬_•[ºˆ§l∂}√ˆ
				...
				else
				{
					...
					OrgTpOutPwm[i] = 0;
					OrgTpOutPwmBk[i] = 0;
					TpPIDdata[i].Isum1 = 0;   // IÁ¥ØË®àÂÄº
					HeatRunFg &= ~(0x01<<i);
...
unsigned short TwoUpTempMd(unsigned short Sec, unsigned short Setting, unsigned short Current, unsigned short Offset)
{
	...
	if((InitTpHt >= (TpMaxLp*2)) && (!(TwoTpHeatFg & (0x01<<Sec))))
	{
		InitTpHt = TpMaxLp*3;
		//
		if(Current >= Setting)		// •ÿ´e≠»§j©Û≥]©w≠»?	
		{
			if((Current >= Setting)	&& (Current <= (Setting2+Offset)))	// •ÿ´e≠»§j©Û≥]©w≠»?	
			{
				if(((TwoTpFg & (0x01<<Sec)) == 0) || ((TwoTpHeatFg & (0x01<<Sec)) == 0))
				{
					...
					{
						if(TwoUpTm[Sec]++ >= 20) // 2014/08/15 ßP¬_¶∏º∆±q10ßÛßÔ¨∞20.
						{
							TwoUpTm[Sec] = 20;
...
void TpAuMdSub(unsigned short Sec)
{
	...
	i = Sec;
	//	Fu 106/07/27
	//if(*(tempData[0].ThermostatFun) & 0x8000)	// 15 Bit : MJ86 - 12¨ÌPIDº“¶°
	// Fu 108/01/03 : •i•H®C¨qøW•ﬂøÔæ‹MT12º“¶°©Œ¨OMJ86º“¶°
	if((*(tempData[0].ThermostatFun) & 0x8000) && ((*tempData[0].TwoUpFun & (0x01<<i)) == 0))	// 15 Bit : MJ86 - 12¨ÌPIDº“¶°
	{
		if((AUTO_FG2 & (0x01<<i)) != 0)
					...
					if(((thermal_couple[Sec]+5u) < *(tempData[Sec].TpControl)) || (TempTmOKFg & (0x01<<Sec)))
					{
						if(TpDownCnt[Sec]++ > 2u)	// 250ms * 3 = 0.75 sec
						{
							if(TempAuTpUnitUp[Sec] < thermal_couple[Sec])
									TempAuTpUnitUp[Sec] = thermal_couple[Sec];	// ∞Oø˝•ÿ´e≥Ã§j∑≈´◊≠»
							//
							TempTmOKFg |= (0x01u<<Sec);
							//
							//TempTm5Min2[Sec]++;
							...
							//{
									...
									//
									Data = TempAuTpUnitUp[Sec]-TempAuTpUnit[Sec];	// ∞Oø˝≥Ã§j™iÆp©M™i®¶™∫Æt≠»
									Data = (Data + ((Data * 5) / 10));
									//
...
unsigned short TpCloseLoopSub(unsigned short sec, unsigned short ChgPDUnitFg)
{
	...
	//	Fu 106/07/27
	// Fu 108/01/03 : •i•H®C¨qøW•ﬂøÔæ‹MT12º“¶°©Œ¨OMJ86º“¶°
	if((*(tempData[0].ThermostatFun) & 0x8000) && ((*tempData[0].TwoUpFun & (0x01<<sec)) == 0))	// 15 Bit : MJ86 - 12¨ÌPIDº“¶°
	{
		/*
		...
		*/
		// Fu 106/07/25
		TpPIDdata[sec].Err = (float)(TpPIDdata[sec].Set) - (float)(thermal_couple[sec]);   // Err = Tset - Tcurr  // Áï∂Ê¨°Ë™§Â∑Æ = Ë®≠ÂÆöÂ£ìÂäõ - ÁõÆÂâçÂ£ìÂäõ
	...
	else
	{
		...
		if(j & (0x01<<sec))
		{
			dXi = (float)(12. * 60.) * (float)0.001;		// 8¨q*±Ω¥y∂g¥¡(760 = 6000/8)
			dXd = (float)(12. * 60.) * (float)0.001;		// 8¨q*10ms±Ω¥y∂g¥¡
		...
		{
			dXi = (float)(12 * 265)*(float)0.001;		// 8¨q*10ms±Ω¥y∂g¥¡
			dXd = (float)(12 * 265)*(float)0.001;		// 8¨q*10ms±Ω¥y∂g¥¡
		...
		TpPIDdata[sec].Err = (float)(TpPIDdata[sec].Set) - (float)(thermal_couple[sec]);   // Err = Tset - Tcurr  // Áï∂Ê¨°Ë™§Â∑Æ = Ë®≠ÂÆöÂ£ìÂäõ - ÁõÆÂâçÂ£ìÂäõ
		//
		// 2014.02.07 ΩT´O®ÏπF≤ƒ§@¶∏≥]©w•ÿº–´·, P≠»¿Hµ€ª~ÆtßÔ≈‹•\Ø‡§~∂}±“.
		...
		{ //within PB
			...
			else
			{
				...
				//if((Output100Pr[sec] != 0) && (TpPIDdata[sec].Err >= 0))
				//{
					//TpPIDdata[sec].Pturn = ((TpPIDdata[sec].Err) * 32767) / (float)(((float)TpPIDdata[sec].P/(float)Output100Pr[sec]));// PÈ†ÖË®àÁÆóÂÄº = Áï∂Ê¨°Ë™§Â∑Æ*32767
				...
				//else
				//{
					TpPIDdata[sec].Pturn = ((TpPIDdata[sec].Err) * 32767) / (float)(TpPIDdata[sec].P);// PÈ†ÖË®àÁÆóÂÄº = Áï∂Ê¨°Ë™§Â∑Æ*32767
			...
			//
			TpPIDdata[sec].Isum1 += TpPIDdata[sec].Iturn;	// IÁ¥ØË®àÂÄº = IÁ¥ØË®àÂÄº + IÈ†ÖË®àÁÆóÂÄº
			TpPIDdata[sec].Out =  TpPIDdata[sec].Isum1 + TpPIDdata[sec].Pturn - TpPIDdata[sec].Dturn;
			//
			if((TpPIDdata[sec].Out > OutputPwmMaxUnit[sec]) || (TpPIDdata[sec].Out < Upmin))// OutputPwmMaxUnit = 32767, Upmin = 0
			{
		...
		//	Fu 106/05/22
		//if((TpPIDdata[sec].Err >= 0) && (TwoTpHeatFg & (0x01<<sec)) && (((*(tempData[0].SynchronFun) & (0x01<<sec)) == 0) || (((*(tempData[0].SynchronFun) & (0x01<<sec)) != 0) && (TpRangCnt >= 48)))) 	// §G¨q§…∑≈§w®ÏπF : Fu 107/11/01 πF®ÏÆ∆∑≈§∫§~∞µ≠p∫‚
		//{
			//if((Output100Pr[sec] != 0) && (thermal_couple[sec] <= TpPIDdata[sec].OldCurr))	// Fu 106/06/01
			//{
				//TpPIDdata[sec].Out = OutputPwmMaxUnit[sec];
		...
		//{
			//	Fu 106/05/31
			//if(TwoTpHeatFg & (0x01<<sec))	// §G¨q§…∑≈§w®ÏπF
				//{	// Fu 108/06/24 : ßP¬_¨Oß_¨∞≥]©w∑≈§Ò≤{¶b∑≈∞™
...
void TempAutoDateRun(void)
{
	...
	else if(autopreheatosh) // πqºˆ´O∑≈(Keep Temperature)
	{
		heatmode = 2;
		autopreheatosh = 0;
	...
	//
	showweek = *tempData[0].Week; // §È¥¡Æ…∂°_New(∂g)
	//
	autoheat = 0;
	autopreheat = 0;
	//
